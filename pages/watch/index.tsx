...
const Watch = () => {
  ...
  return (
    <MainLayout dark fullLayout={fullLayout} nowPlaying={nowPlaying}>
      {query.isLoading || queryMedia.isLoading ? (
        <div>
          <p>Loading...</p>
        </div>
      ) : query.isError || queryMedia.isError ? (
        <div>
          <h3>Could not play media </h3>
        </div>
      ) : (
        <>
          <WatchWindow
